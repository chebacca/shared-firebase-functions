...
export const updatePayment = onRequest({ memory: '512MiB' }, async (req: any, res: any) => {
  try {
    // Set CORS headers
...
export const updatePaymentCallable = onCall(defaultCallableOptions, async (request) => {
  return await updatePaymentLogic(request.data, undefined);
});