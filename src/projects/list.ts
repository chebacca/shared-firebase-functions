...
export const listProjects = onRequest({ memory: '512MiB' }, async (req: any, res: any) => {
  try {
    // Set CORS headers
...
export const listProjectsCallable = onCall(defaultCallableOptions, async (request) => {
  return await listProjectsLogic(request.data, undefined);
});