...
/**
 * HTTP endpoint for Box OAuth callback
 * This is called by Box after user authorizes the app
 */
export const boxOAuthCallback = onRequest(
  {
    region: 'us-central1',
    cors: true,
    secrets: [encryptionKey],
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        ...
        return res.redirect('https://clipshowpro.web.app/integration-settings?box_error=authorization_failed');
      }

      if (!code || !state) {
        return res.redirect('https://clipshowpro.web.app/integration-settings?box_error=missing_parameters');
      }
      ...
      if (stateDoc.empty) {
        return res.redirect('https://clipshowpro.web.app/integration-settings?box_error=invalid_state');
      }
      ...
      return res.redirect('https://clipshowpro.web.app/integration-settings?box_error=callback_failed');
    }
  }
);
...
async function completeOAuthCallback(code: string, state: string, stateData: any) {
  try {
    ...
    const tokenInfo = await boxSDK.getTokensAuthorizationCodeGrant(code, {
      redirectURI: redirectUri || boxConfig.redirectUri
    });
    ...
    return 'https://clipshowpro.web.app/integration-settings?box_connected=true';
