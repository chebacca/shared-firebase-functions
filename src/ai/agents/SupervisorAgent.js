...
const DataQueryAgent_1 = require("./DataQueryAgent");
const ActionExecutionAgent_1 = require("./ActionExecutionAgent");
const PlanningAgent_1 = require("./PlanningAgent");
const ReportGenerationAgent_1 = require("./ReportGenerationAgent");
class SupervisorAgent {
    ...
    constructor(ollamaService, toolRegistry) {
        ...
        this.queryAgent = new DataQueryAgent_1.DataQueryAgent(ollamaService, toolRegistry);
        this.actionAgent = new ActionExecutionAgent_1.ActionExecutionAgent(ollamaService, toolRegistry);
        this.planningAgent = new PlanningAgent_1.PlanningAgent(ollamaService, toolRegistry);
        this.reportAgent = new ReportGenerationAgent_1.ReportGenerationAgent(ollamaService, toolRegistry);
    }
    /**
        ...
        try {
            ...
            switch (routing.agent) {
                ...
                case 'report':
                    ...
                    result = await this.reportAgent.generateReport(userRequest, context.projectData, context);
                    break;
                default:
     ...
     */
    ruleBasedClassification(userRequest) {
        const lowerRequest = userRequest.toLowerCase();
        // Check for report intent
        if (ReportGenerationAgent_1.ReportGenerationAgent.isReportIntent(userRequest)) {
            return {
                agent: 'report',
                confidence: 0.9,
        ...
        if (DataQueryAgent_1.DataQueryAgent.isQueryIntent(userRequest)) {
            return {
                agent: 'query',
                confidence: 0.85,
        ...
        if (ActionExecutionAgent_1.ActionExecutionAgent.isActionIntent(userRequest)) {
            return {
                agent: 'action',
                confidence: 0.85,
        ...
        if (PlanningAgent_1.PlanningAgent.isPlanningIntent(userRequest)) {
            return {
                agent: 'planning',
                confidence: 0.85,
     ...
     */
    async llmBasedClassification(userRequest) {
