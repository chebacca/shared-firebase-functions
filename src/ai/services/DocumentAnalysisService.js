...
class DocumentAnalysisService {
    ...
    constructor() {
        ...
        if (this.useOllama) {
            try {
                ...
                this.ollamaService = new OllamaAnalysisService_1.OllamaAnalysisService();
    ...
    async analyzeProject(projectData, options) {
        ...
        // Determine effective Ollama usage
        const effectiveUseOllama = options.useOllama !== undefined ? options.useOllama : this.useOllama;
        // Try Ollama first if enabled (either globally or via option)
        if (effectiveUseOllama) {
            // Lazy initialize if not already done
            if (!this.ollamaService) {
                try {
                    console.log('[DocumentAnalysisService] 🤖 Lazy initializing Ollama service (requested by options)...');
                    this.ollamaService = new OllamaAnalysisService_1.OllamaAnalysisService(options.ollamaBaseUrl);
                    console.log('[DocumentAnalysisService] ✅ Ollama service initialized successfully');
                }
                catch (error) {
                    console.warn('[DocumentAnalysisService] ⚠️ Ollama lazy initialization failed:', error);
                }
            }
            if (this.ollamaService) {
                try {
...
${Object.entries(analytics.deliverableStatusBreakdown || {}).map(([status, count]) => `- ${status}: ${count} deliverables`).join('\n')}
...
${Object.entries(analytics.sessionPhases || {}).map(([phase, count]) => `- ${phase}: ${count} sessions`).join('\n')}
- Total Sessions: ${analytics.totalSessions || 0}

### Team by Department:
${Object.entries(analytics.teamByDepartment || {}).map(([dept, data]) => `- ${dept}: ${data.count} team members`).join('\n')}

### Labor Costs by Department:
${Object.entries(analytics.timecardByDepartment || {}).map(([dept, data]) => `- ${dept}: ${data.hours.toFixed(1)} hours, $${data.cost.toLocaleString()} cost, ${data.count} timecards`).join('\n')}

### Expenses by Category:
${Object.entries(analytics.expenseByCategory || {}).map(([cat, data]) => `- ${cat}: $${data.amount.toLocaleString()} (${data.count} expenses)`).join('\n')}

### Workflow Status:
${Object.entries(analytics.workflowStepStatus || {}).map(([status, count]) => `- ${status}: ${count} steps`).join('\n')}
...
`;
      ...
      - Paid/Approved expenses: ${expenses.filter((e) => e.status === 'paid' || e.status === 'approved').length}
      ...
      - Paid/Approved batches: ${payrollBatches.filter((p) => p.status === 'paid' || p.status === 'approved').length}
    ...
    `;
        ...
        try {
            ...
            console.warn('[DocumentAnalysisService] ⚠️ Standard JSON parse failed, attempting cleanup...');
            try {
                // Formatting repair: remove comments, fix trailing commas
                let cleaned = jsonText
                    .replace(/\/\/.*$/gm, '') // Remove single-line comments
                    .replace(/\/\*[\s\S]*?\*\//g, '') // Remove multi-line comments
                    .replace(/,(\s*[}\]])/g, '$1'); // Remove trailing commas
                insights = JSON.parse(cleaned);
                console.log('[DocumentAnalysisService] ✅ JSON parsed successfully after cleanup');
            }
            catch (repairError) {
                console.error('[DocumentAnalysisService] ❌ Failed to parse JSON even after cleanup:', repairError);
                console.error('[DocumentAnalysisService] Raw text snippet:', text.substring(0, 500) + '...');
                throw new Error('Failed to parse AI response as JSON: ' + (repairError instanceof Error ? repairError.message : String(repairError)));
            }
        }
