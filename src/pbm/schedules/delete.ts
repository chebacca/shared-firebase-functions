...
// ============================================================================
// PBM SCHEDULE DELETION
// ============================================================================

export const deleteSchedule = functions.https.onRequest(async (req, res) => {
  try {
    // Set CORS headers
    ...
    // Delete the schedule
    await admin.firestore().collection('pbmSchedules').doc(scheduleId).delete();

    console.log(`✅ [DELETE PBM SCHEDULE] Deleted schedule: ${scheduleId}`);

    res.status(200).json(createSuccessResponse({
      scheduleId
    }, 'PBM schedule deleted successfully'));

  } catch (error: any) {
    console.error('❌ [DELETE PBM SCHEDULE] Error:', error);
    res.status(500).json(handleError(error, 'deletePBMSchedule'));
  }
});
