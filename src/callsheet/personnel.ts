 ...
 */
export const callsheet_createPersonnelAccount = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  try {
 ...
 */
export const callsheet_changePersonnelPassword = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  try {
    // Verify user is authenticated
    if (!context.auth?.uid) {
      return { success: false, error: 'User not authenticated' };
    ...
    console.log(`✅ Password changed for personnel: ${context.auth.uid}`);
 ...
 */
export const callsheet_resetPersonnelPassword = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  try {
    ...
    await personnelQuery.docs[0].ref.update({
      ...
      resetByAdmin: context.auth.uid,
      updatedAt: new Date()
    });
    
    console.log(`✅ Password reset for personnel: ${personnelUid} by admin: ${context.auth.uid}`);
