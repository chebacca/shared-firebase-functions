...
export const deleteDataset = onRequest({ memory: '512MiB' }, async (req: any, res: any) => {
  try {
    // Set CORS headers
...
export const deleteDatasetCallable = onCall(defaultCallableOptions, async (request) => {
  return await deleteDatasetLogic(request.data, undefined);
});